...
// ---- lan 命令（Windows 开机自启管理 serve）----
// 双机制：① schtasks 计划任务（默认，需相应权限）；② 非管理员（Access denied）自动降级为
// 用户级 Startup 静默自启（VBS sh.Run <autostart.cmd>, 0, False + autostart.cmd；node 路径 process.execPath 自动探测，0.5.4）
...
function usage() {
  console.log([
    ...
    '  yotta-memory lan enable [--onstart] | disable | status  开机自启管理（Windows 计划任务，默认 ONLOGON；非管理员自动降级用户级 Startup 静默自启）',
