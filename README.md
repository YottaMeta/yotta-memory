...
> 🆕 **v0.17.1 (view delete confirmation feedback)**: when deleting an AI identity in `view`, a mismatched confirmation ID now shows an immediate error instead of returning silently. Cancel remains silent, and the server-side confirmation and destructive-operation gates are unchanged.
