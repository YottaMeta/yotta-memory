...
- **隔离由机制保证**：FACT 进公共区共享，PREF / BOUND / COMMIT 进私密区、按 agent 隔离。读取按 scope/owner 分区过滤，越界读取由 CLI 硬性拒绝——不依赖智能体的「自觉」。
...
| **真正的权限边界** | 公共 / 私密分区 + scope/owner 过滤 + 授权机制（grant / identity=user / `--unsafe`），越界读被硬性拒绝，不靠 AI 自律 |
